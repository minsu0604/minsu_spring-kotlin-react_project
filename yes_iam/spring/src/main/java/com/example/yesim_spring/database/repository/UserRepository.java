...
public interface UserRepository extends JpaRepository<UserEntity, Long> {
    ...
    @Query(value = "select * from user where approved_yn = \"Y\" and not(user_seq = 1) order by case when role = \"ROLE_SENIOR_MANAGER\" then 0 when role = \"ROLE_MANAGER\" then 1 else 2 end", nativeQuery = true)
